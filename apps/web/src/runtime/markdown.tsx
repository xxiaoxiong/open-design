...
function renderInline(
  ...
  while ((m = re.exec(text))) {
    ...
    if (m[1]) {
      ...
      const resolved = options?.resolveProjectFileLink?.(m[3]);
      out.push(
        <a
          key={key++}
          className="md-link"
          href={resolved ? undefined : m[3]}
          onClick={
            resolved
              ? (e) => {
                  e.preventDefault();
                  options?.onOpenProjectFile?.(resolved);
                }
              : undefined
          }
          {...(resolved ? {} : { target: '_blank', rel: 'noreferrer noopener' })}
        >
          {m[2]}
        </a>,
      );
    } else if (m[4]) {
      const resolved = options?.resolveProjectFileLink?.(m[4]);
      out.push(
        <a
          key={key++}
          className="md-link md-link-bare"
          href={resolved ? undefined : m[4]}
          onClick={
            resolved
              ? (e) => {
                  e.preventDefault();
...
function pushText(
  ...
  while ((m = urlRe.exec(text))) {
    ...
    const resolved = options?.resolveProjectFileLink?.(m[1]);
    segments.push(
      <a
        key={`${baseKey}-${k++}`}
        className="md-link"
        href={resolved ? undefined : m[1]}
        onClick={
          resolved
            ? (e) => {
                e.preventDefault();
